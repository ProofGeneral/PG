...
;; don't use save-excursion when possible, changing point is expensive
(defun proof-set-sent-end (end)
  "Set end of sent region to END."
  (let* ((pos end)
	...
	(let ((found-comment t))
	  (while found-comment
		...
		(cl-incf pos)
		(setq ch (char-after pos))))))))
    ;; find end of last statement or comment
    (when (> pos bof)
      (cl-decf pos))
    (setq ch (char-after pos))
    (while (and (> pos bof)
...
(defun proof-not-in-sent-region (start end)
  ;; called by read-only hook for proof-locked-span, to see if either:
  ;;  START in read-only region within proof-sent-spant established by
  ;;   proof-retract-point-limit, or
  ;;  START and END are beyond sent region
  ...
  (when (and proof-locked-span
    ...
    (let ((sent-end (span-end proof-sent-span)))
      (or (and proof-retract-point-limit
	       (<= start (- sent-end proof-retract-point-limit)))
...
Also clear list of script portions."
  ...
  (if proof-locked-span
      (proof-set-locked-endpoints 1 1)
  ...
  (mapcar
   (lambda (buffer)
     (save-excursion
       ...
       (when (eq buffer proof-script-buffer)
...
(defun proof-script-delete-spans (beg end)
  "Delete primary spans between BEG and END."
  ...
  (if (span-live-p span)
	...
	(unless (proof-locked-region-empty-p)
	  (save-excursion
	    ...
	    (skip-chars-backward "\n")
	    (setq start (point))
	    ;; proof-set-sent end returns end of preceding statement
	    ;; use that to set locked-end
	    (let ((stmt-end (proof-set-sent-end start)))
	      (proof-set-locked-end stmt-end))
...
(defun proof-script-after-change-function (start _end _prelength)
  ...
  (setq proof-last-edited-low-watermark
	(min (or proof-last-edited-low-watermark (point-max))
	     start))
  (when (and (markerp proof-overlay-arrow)
