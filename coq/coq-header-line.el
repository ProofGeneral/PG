...
;; table of colored items on header line to buffer positions
(defvar coq-header-line--buffer-pos-tbl nil)
...
(defun coq-header-line--init-buffer-pos-tbl ()
  (setq coq-header-line--buffer-pos-tbl (make-hash-table))) 
  ...
  (when coq-use-header-line
	...
	(condition-case err-msg
 	    (when (coq-header-line--need-update)
	      ...
	      (coq-header-line--init-buffer-pos-tbl)
		...
		(dolist (span (spans-all))
		  (pcase (span-property span 'type)
		    ...
		    (`pg-error ; this type includes both errors and warnings
		     (pcase (span-property span 'face)
		       (`proof-error-face
			(cl-incf error-count)
			(setq error-spans (cons span error-spans)))
		       (`proof-warning-face
			(setq warning-spans (cons span warning-spans)))
		       (_ (error "Unknown face in span of type 'pg-error"))))))
		...
		(let ((sorted-colored-spans (sort colored-spans (lambda (sp1 sp2) (< (span-property sp1 'priority)
		  ...
		  (dolist (span sorted-colored-spans)
		    (let* ((span-info (coq-header-line--span-info span num-cols num-lines))
			   ...
			   (header-line-width (- end start))
			   (span-width (- (span-end span) (span-start span)))
			   (span-incr (/ span-width (float header-line-width))))
		      (if (display-graphic-p)
			...
			(coq-header-line--add-text-face start end `(:background ,color) header-text))
		      ;; add entry to buffer-pos table
		      (dotimes (offs header-line-width)
			(let* ((header-pos (+ start offs))
			       (buff-pos (+ (span-start span) (round (* offs span-incr)))))
			  (puthash header-pos buff-pos coq-header-line--buffer-pos-tbl))))))
		...
		;; do warnings, then errors, so errors will overwrite if overlap
		(dolist (span (append warning-spans error-spans))
		  (let* ((span-info (coq-header-line--span-info span num-cols num-lines))
			 (new-face (coq-header-line--span-info-face span-info))
			 (color (coq-header-line--span-info-color span-info))
			 (start (coq-header-line--span-info-start span-info))
...
(defun coq-header-line-mouse-handler ()
  ...
  (let ((event last-input-event))
	...
	(when (and x-pos (eq major-mode 'coq-mode) (eq mouse-info 'mouse-1))
	  ;; use entry in buffer-pos table, if possible
	  (let ((buffer-pos (gethash x-pos coq-header-line--buffer-pos-tbl)))
	    (if buffer-pos
		(goto-char buffer-pos)
	      ;; else calculate approximate position
