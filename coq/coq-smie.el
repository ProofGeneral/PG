...
(defun if-def (symb) (or (and (boundp symb) (symbol-value symb)) nil))
...
(defun coq-debug-smie-rule (point kind parent token ident-parent &optional is-sibling is-hanging)
  (message "###############################")
  (message
   "** %s %S at %S / (token: %S)"
   (pcase kind
     (':after "AFTER") (':before "BEFORE") (':list-intro "LIST-INTRO")
     (':elem "ELEM") (':close-all "CLOSE-ALL") (_ "??unknown kind??"))
   (pcase kind
     (':after (coq-indent-categorize-token-after token))
     (':before (coq-indent-categorize-token-before token))
     (_ token))
   (point) token)
  (message  "    (sibling=%S, parent=%S, hanging: %S, id-parent: %S)"
            is-sibling parent is-hanging ident-parent)
  (when ident-parent
    (hlt-highlight-regions (list (list (cadr ident-parent) (+ (cadr ident-parent) 1)))
                           'hlt-property-highlight)
    (hlt-highlight-regions (list (list (cadr ident-parent) (+ (cadr ident-parent) 1)))
                           'underline))
  (let* ((pt (point)) (prt parent)
         (msg (lambda (n) (coq-debug-smie--parent pt prt n))))
    (pcase kind (':after (funcall msg 1)) (':before (funcall msg 2))
           (':close-all (funcall msg 2))
           (':list-intro (funcall msg 2)) (':elem (funcall msg 2)))))
...
(defun coq-smie-rules-indent (kind token)
...
(defvar coq-smie--debug nil
  "internal use: graphical debuging of proofgeneral indentation.")
(defvar coq-smie--debugfile nil
  "internal use: graphical debuging of proofgeneral indentation.")
...
(defun coq-smie-rules (kind token)
  ;; debug
  (if coq-smie--debug
      (coq-debug-smie-rule (point) kind (if-def 'smie--parent) token
                           (and (boundp 'smie--parent) (smie-indent--parent))
                           (and (boundp 'smie--parent) (smie-rule-sibling-p))
                           (and (boundp 'smie--parent) (smie-rule-hanging-p))))
  (let ((res (coq-smie-rules-indent kind token)))
    (if coq-smie--debug (message "RES (%S) = %S" token res))
    res))

(defun coq-smie--debug-goto-char ()
  (interactive)
  (let ((n (number-at-point)) (win (get-buffer-window coq-smie--debugfile)))
    (message "WIN: %S" win)
    (save-selected-window
      (select-window win)
      (goto-char n))))

(defun coq-smie--debug()
  (interactive)
  (let ((buf (current-buffer)))
    (setq coq-smie--debug t)
    (setq coq-smie--debugfile buf)
    (display-buffer "*Messages*" t)
    (global-set-key [(f7)]  'coq-smie--debug-goto-char)
    (global-set-key [(shift f7)]  'hlt-unhighlight-region)))
